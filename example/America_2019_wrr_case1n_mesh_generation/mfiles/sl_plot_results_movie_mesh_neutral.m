...
for n=1:1:length(nod)-1
  ...
  number_neg_conc=sum(loc_neg_conc);
  if number_neg_conc>0 
      sprintf(' %d negative concentration points, the first one is at x= %e, y= %e \n', number_neg_conc , nod(n).terms{xnod_idx}(loc_neg_conc(1)), nod(n).terms{ynod_idx}(loc_neg_conc(1)) );
  end


  % output smallest P
  [minimum_p,minimum_p_idx]=min(nod(n).terms{c_idx})
  sprintf('the minimum p is %e, located in x= %e , y = %e ' min(nod(n).terms{p_idx}, nod(n).terms{p_idx}(minimum_p_idx), nod(n).terms{ynod_idx}(minimum_p_idx) );
...
%   %%  plot saturation over time
  ...
  contourf ( x_nod_mtx,y_nod_mtx,real(log10(c_mtx)),'LevelStep',0.05,'LineStyle','None');hold on   % !warning this will disable all the negative concentrations
