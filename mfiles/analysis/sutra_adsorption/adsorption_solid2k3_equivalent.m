%This script demonstrates using freundlich adsoprtion model to simulation solid
% salt due to evaporation. it was found that 'solid 500' works well. 
% now we use equivalent adsorption 
clear
...
pf(i).adsp=adsorption_freundlich(c,pf(i).chi1,pf(i).chi2);
pf(i).str=['freundlich, chi1=',num2str(pf(i).chi1),', chi2=',num2str(pf(i).chi2)];
i=11;pf(i).chi1=1.5e-13  ;pf(i).chi2=0.14;
pf(i).adsp=adsorption_freundlich(c,pf(i).chi1,pf(i).chi2);
pf(i).str=['freundlich, chi1=',num2str(pf(i).chi1),', chi2=',num2str(pf(i).chi2)];
...
i=15;pf(i).chi1=2e-45;  ;pf(i).chi2=0.05;
pf(i).adsp=adsorption_freundlich(c,pf(i).chi1,pf(i).chi2);
pf(i).str=['freundlich, chi1=',num2str(pf(i).chi1),', chi2=',num2str(pf(i).chi2)];
...
plot(c,adsp_linear,'m:','linewidth',2,'displayname'...
    ,'linear2000');hold on
...
saveas(h,'solid2k_equivalent.fig','fig');


print(h,'-djpeg','-r300','solid2k_equivalent.jpg')
