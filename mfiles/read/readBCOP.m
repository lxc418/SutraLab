function [o,o2]=readBCOP(varargin)
  ...
  % Example:
  % [bcopdata,nodhead]=readBCOP('project','outputnumber',3);
  %    Purpose: parsing 'project.bcop' (or 'project.BCOP')
  ...
  % ---------------- Parsing the line with bcop, element info-----------------
  ...
  for n=output_from:output_no
    ...
    if tmp  ~= -1  % check if simulation is incomplete
      ...
      % get o.nbcp as it is not disclosed in *.bcop
      if n==output_from 
        [tmp o2.bcop] = getBlock(fn,'keyword','#');
        tmp = textscan(tmp,'%f %s %s %f %f %f %f %f',o2.bcop);
      else
        tmp = textscan(fn ,'%f %s %s %f %f %f %f %f',o2.bcop);
      end
