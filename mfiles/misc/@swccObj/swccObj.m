...
% http://au.mathworks.com/help/matlab/matlab_oop/comparing-handle-and-value-classes.html
  ...
  methods    % seems that all functions in methods needs to be called.
    function o=swccObj(varargin)
      ...
      tmp=getNextLine(fn,'criterion','without','keyword','#','ignoreblankline','yes');
      i=1;
      b=struct;
      while tmp~=-1
          b(i).inp=tmp;
          tmp=getNextLine(fn,'criterion','without','keyword','#','ignoreblankline','yes');
          %tmp     = textscan(tmp,'%s %f %f %f %f %f %s %s','delimiter',',');
	  %[o.swres,o.aa1,o.vn,o.ksat,o.por]=deal(tmp{2:6});
	  i=i+1;
      end
      ...
      for i = 1:o.no_session
          tmp     = textscan(b(i).inp,'%s %f %f %f %f %f %f %s %s','delimiter',',');
	  [o.swres(i),o.aa1(i),o.vn(i),o.ksat(i),o.por(i),o.psim0(i)]=deal(tmp{2:7});
	  %o.soil_type{i}=tmp{1};  % this is one of the working solution but not very good
      o.soil_type={o.soil_type{:},tmp{1}{:}};
      o.comment ={o.comment{:},tmp{8}{:}};
      o.line_type ={o.line_type{:},tmp{9}{:}};
      end
